...
void main()
{
	...
	if(!normalMapping)
	{
		...
		// position vector and the fragment's position vector. We can easily calculate
	...
	/*
		...
		to make sure the diffuse component (and thus the colors) never become negative. From 0 to 1
	*/
	...
	// A specular intensity value to give the specular highlight a medium-bright color so that it doesn't have too much of an impact
	float specularStrength = 0.2f;
	float spec = 0.0f;
	/*
		Unit vector exactly halfway between the view direction and the light direction. 
		The closer this halfway vector aligns with the surface's normal vector, the higher 
		the specular contribution.When the view direction is perfectly aligned with 
		the (now imaginary) reflection vector, the halfway vector aligns perfectly with 
		the normal vector. Thus the closer the viewer looks in the original reflection direction, 
		the stronger the specular highlight becomes. Here you can see that whatever direction
		the viewer is look from, the angle between the halfway vector and the surface
		normal never exceeds 90 degrees (unless the light is far below the surface of course). 
		This produces slightly different results compared to Phong reflections, but mostly 
		looks slightly more visually plausible, especially with low specular exponents.
	*/
	vec3 halfwayDirection = normalize(lightDirection + viewDirection); 
	/*
		Another subtle difference between Phong and Blinn-Phong shading is that the angle between the halfway
		vector and the surface normal is often shorter than the angle between the view and reflection vector. 
		As a result, to get similar results to Phong shading the specular shininess exponent has to be set a bit higher. 
